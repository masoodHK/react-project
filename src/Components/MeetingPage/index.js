    ...
    firestore
      ...
      .then(users => {
        ...
        const filteredData = data.filter(
          user =>
            this.compare(
              user.typeOfDrinks.sort(),
              userData.typeOfDrinks.sort()
            ) ||
            this.compare(
              user.timeInMinutes.sort(),
              userData.timeInMinutes.sort()
            )
        );
      ...
      <div id="cards">
        ...
        >
          {data.map((item, index) => (
            ...
            >
              <img
                src={item.imageUrls ? item.imageUrls[0] : placeholder}
                alt="Display"
              />
              <div id="bottom">
                <Button
                  className="card-button"
                  onClick={() => this.action("swipe left")}
                >
                  Reject
                </Button>
                <div className="info">
